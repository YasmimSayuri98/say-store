...
// (BACKLOG) baixarEtiquetas da Shopee existe em plataformaSyncService/adapter, mas está desligado por ora.
...
// Lote: conclui a "Programação de envio" e avança os pedidos para a Produção.
// (BACKLOG) O download da etiqueta oficial da Shopee via API existe no adapter, mas está desligado
// por ora — aqui apenas marcamos etiquetaGerada para o pedido seguir para a produção.
router.post('/lote/etiquetas', async (req, res, next) => {
  try {
    ...
    res.json({ processados: r.count, pdfBase64: null });
  } catch (e) { next(e); }
});
